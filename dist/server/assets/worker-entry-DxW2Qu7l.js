...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-BiZ9VLV2.js");
...
const handleServerAction = async ({
  ...
  function parsePayload(payload) {
    const parsedPayload = Ou(payload, { plugins: serovalPlugins });
        ...
        if (FORM_DATA_CONTENT_TYPES.some(
          ...
          if (typeof serializedContext === "string") {
            try {
              ...
              const deserializedContext = Ou(parsedContext, {
...
async function loadEntries() {
  const routerEntry = await import("./router-Dohyv0--.js").then((n) => n.b);
