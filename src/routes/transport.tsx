    ...
    <DashboardLayout>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Bus routes, drivers & student allocation</p>
        </div>
        <Button onClick={() => { setEditRoute(null); setForm({ routeNo: "", from: "", to: "", via: "", driverName: "", driverPhone: "", busNo: "", capacity: 50, departureTime: "", arrivalTime: "" }); setAddDialog(true); }} className="gap-2"><Plus className="h-4 w-4" /> Add Route</Button>
      </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <Badge className="bg-primary text-primary-foreground">{route.routeNo}</Badge>
                    <h3 className="font-semibold text-foreground">{route.from} → {route.to}</h3>
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">Via: {route.via}</p>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="flex items-center gap-2 text-muted-foreground"><Bus className="h-3.5 w-3.5" /> Bus: {route.busNo}</div>
                    <div className="flex items-center gap-2 text-muted-foreground"><User className="h-3.5 w-3.5" /> Driver: {route.driverName}</div>
                    <div className="flex items-center gap-2 text-muted-foreground"><Phone className="h-3.5 w-3.5" /> {route.driverPhone}</div>
                    <div className="flex items-center gap-2 text-muted-foreground"><MapPin className="h-3.5 w-3.5" /> {route.departureTime} — {route.arrivalTime}</div>
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">Students: {route.assignedStudents.length}/{route.capacity}</span>
                    <div className="h-2 w-32 rounded-full bg-secondary">
                      <div className="h-2 rounded-full bg-primary" style={{ width: `${(route.assignedStudents.length / route.capacity) * 100}%` }} />
                    </div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setAllocateDialog(route)}>Allocate</Button>
                  <Button variant="ghost" size="sm" onClick={() => openEdit(route)}><Pencil className="h-3.5 w-3.5" /></Button>
                  <Button variant="ghost" size="sm" onClick={() => deleteRoute(route.id)} className="text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Dialog open={addDialog} onOpenChange={setAddDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-sm font-medium text-foreground">Route No *</label><Input value={form.routeNo} onChange={e => setForm({ ...form, routeNo: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">Bus No</label><Input value={form.busNo} onChange={e => setForm({ ...form, busNo: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">From *</label><Input value={form.from} onChange={e => setForm({ ...form, from: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">To *</label><Input value={form.to} onChange={e => setForm({ ...form, to: e.target.value })} /></div>
            <div className="col-span-2"><label className="text-sm font-medium text-foreground">Via</label><Input value={form.via} onChange={e => setForm({ ...form, via: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">Driver Name *</label><Input value={form.driverName} onChange={e => setForm({ ...form, driverName: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">Driver Phone</label><Input value={form.driverPhone} onChange={e => setForm({ ...form, driverPhone: e.target.value })} /></div>
            <div><label className="text-sm font-medium text-foreground">Departure</label><Input value={form.departureTime} onChange={e => setForm({ ...form, departureTime: e.target.value })} placeholder="8:00 AM" /></div>
            <div><label className="text-sm font-medium text-foreground">Arrival</label><Input value={form.arrivalTime} onChange={e => setForm({ ...form, arrivalTime: e.target.value })} placeholder="8:45 AM" /></div>
            <div><label className="text-sm font-medium text-foreground">Capacity</label><Input type="number" value={form.capacity} onChange={e => setForm({ ...form, capacity: Number(e.target.value) })} /></div>
          </div>
          <Button onClick={handleSave} className="w-full mt-2">{editRoute ? "Save Changes" : "Add Route"}</Button>
        </DialogContent>
      </Dialog>
